...
const help = (code: number): never => {
  console.log(`kitty-browser SIXEL raster browser

Usage:
  bun run terminal:sixel -- <url> [options]
...
const percent = (value: number): number => Math.round(value * 100 / 255);

const sixelPalette = (): string => {
  const parts: string[] = [];
  for (let index = 0; index < SIXEL_COLOURS; index += 1) {
    const r = (index >> 5) & 0x07;
    const g = (index >> 2) & 0x07;
    const b = index & 0x03;
    parts.push(`#${index};2;${percent(Math.round(r * 255 / 7))};${percent(Math.round(g * 255 / 7))};${percent(Math.round(b * 255 / 3))}`);
  }
  return parts.join("");
};

const SIXEL_PALETTE = sixelPalette();

const rgb332 = (r: number, g: number, b: number): number =>
  ((r >> 5) << 5) | ((g >> 5) << 2) | (b >> 6);

const sixelRun = (value: number, count: number): string => {
  const ch = String.fromCharCode(63 + value);
  if (count >= 4) return `!${count}${ch}`;
  return ch.repeat(count);
};

const encodeMask = (mask: Uint8Array, last: number): string => {
  if (last < 0) return "";
  const parts: string[] = [];
  let value = mask[0] ?? 0;
  let count = 1;
  for (let x = 1; x <= last; x += 1) {
    ...
    parts.push(sixelRun(value, count));
    value = next;
    count = 1;
  }
  parts.push(sixelRun(value, count));
...
const sixelFrame = (pngBytes: Uint8Array): string => {
  const png = PNG.sync.read(Buffer.from(pngBytes));
  const { width, height, data } = png;
  const parts: string[] = [at(0, 0), `\x1bP0;0;0q"1;1;${width};${height}`, SIXEL_PALETTE];

  for (let bandY = 0; bandY < height; bandY += 6) {
    const masks: Array<Uint8Array | undefined> = new Array(SIXEL_COLOURS);
    const last = new Int32Array(SIXEL_COLOURS);
    last.fill(-1);
    const active: number[] = [];
    ...
    for (let dy = 0; dy < 6; dy += 1) {
      ...
      for (let x = 0; x < width; x += 1) {
        ...
        const colour = alpha < 128
          ? 0
          : rgb332(data[offset] ?? 0, data[offset + 1] ?? 0, data[offset + 2] ?? 0);
        let mask = masks[colour];
        if (!mask) {
    ...
    for (let i = 0; i < active.length; i += 1) {
      const colour = active[i]!;
      const mask = masks[colour]!;
      parts.push(`#${colour}`, encodeMask(mask, last[colour]!));
      if (i + 1 < active.length) parts.push("$");
    }
...
const statusMetadata = (): string => {
  ...
  return `${mode}  ${args.fps}fps  sixel-256  session:${browser.session}  audio:${audio ? "on" : "off"}  ${resolution}  pointer ${cursorX},${cursorY}`;
};
...
const capture = async (): Promise<void> => {
  ...
  try {
    ...
    await stdout(`${mediaFrameMarker(frame, args.fps, captureTimestampNs)}${sixelFrame(screenshot)}`);
    paintStatus();
    frame += 1;
  } catch (error) {
